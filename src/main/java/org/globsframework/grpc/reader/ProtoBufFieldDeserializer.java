 ...
 * indexed by the field number the tag carries; {@code call}, inherited from {@link MutableFunctionWrite}, is
 * what a {@code GeneratedCallerWrite} drives — one call site per field number rather than one for the whole
 ...
 * 229k → 191k ops/s). {@code read} declares IOException and {@link MutableFunctionWrite} declares nothing, so
...
public interface ProtoBufFieldDeserializer extends ProtoBufGlobDeserializer,
        MutableFunctionWrite<SafeHeapReader, Void, Void> {
}
